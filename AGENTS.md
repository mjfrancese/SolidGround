...
The current Linux the infrastructure project runner does not include Autodesk's Revit reference assemblies. The agent must not make Phase 1 CI depend on unofficial repackaged Autodesk binaries. Phase 2 CI is decided (Issue #12, 2026-09-20): once `SolidGround.Revit` exists, the agent must add a CI-only compile gate on the existing `self-hosted` lane, behind a CI-only MSBuild condition named `UseRevitReferenceAssemblies`, referencing the exact-pinned `Nice3point.Revit.Api.RevitAPI`/`RevitAPIUI` 2027.x packages with a `packages.lock.json`, and with `EnableWindowsTargeting` scoped to the same condition. Neither local builds nor the shipped add-in may ever use this condition; local and deploy builds keep referencing the installed Revit 2027 SDK directly. This gate must keep every one of the nine conditions above true and every item on the never-list unchanged: no pull-request trigger, the job-level guard and first-step check carried over, and no new action added to the allow-list. The allow-list stays exactly `actions/checkout` and `actions/setup-dotnet`; `actions/cache` must not be added. See `docs/architecture/revit-add-in-conventions.md` for the full mechanism and the licensing position accepted for this reference-assembly source.
