...
The installed Revit 2027 `SiteDB.dll` contains both `NativeToposolidMaxPointThreshold` and `LinkToposolidMaxPointThreshold`, confirming that the settings still exist in 2027. Autodesk's current Revit 2027 product help restates a 10,000 to 50,000 range and a 20,000 default for both settings on its "Misc Settings in Revit.ini", "About Toposolids", and "Link Topography" pages (Issue #13, item 9); no documentation ties either setting to the `Toposolid.Create` API path, and the installed product's own values are read from `Revit.ini` after the first Revit 2027 launch (manual test step 7 in `docs/architecture/revit-2027-verification-and-host-design.md`). The agent must retain a conservative application default near 15,000, make the budget configurable, and re-verify the numeric limits before Phase 2 behavior depends on them.

## Revit add-in conventions

Issue #12 approved the Phase 2 conventions below on 2026-09-20; `docs/architecture/revit-add-in-conventions.md` records the full the owner's other add-in basis, citations, and the 14 items still needing Revit 2027 verification (Issue #13). Issue #13 verified all 14 of those items on 2026-09-20, with item 2's Undo-stack side effect and several other items' residual runtime-only questions deferred to the manual integration test plan; the results, the locked thin-host design, and that test plan are recorded in `docs/architecture/revit-2027-verification-and-host-design.md`. Issue #15 implemented toposolid creation on 2026-09-21 following these conventions; its design decisions, settings-file template, placement-record schema, error catalogue, Result-code policy, the boundary-Z decision, the Issue #16 extension point, and the manual evidence plan for the verification note's remaining items 2, 9, 15, and 16 are recorded in `docs/architecture/revit-toposolid-creation.md`. The agent must follow these rules for `SolidGround.Revit`:
