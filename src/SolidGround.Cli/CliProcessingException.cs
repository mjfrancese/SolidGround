...
/// raster set or creating its output directory (the raster set has no Core-owned exporter), while writing
/// <c>parcel</c>'s <c>--output</c> files or creating their output directory, or while writing
/// <c>geocode</c>'s <c>--output-file</c>. Always maps to <see cref="CliExitCodes.Processing"/>. See
/// docs/architecture/cli-workflow.md's "Exit codes and error classes" section for the full mapping this
/// exception type participates in, and its "Raster set persistence" section for why raster-set writes need
/// their own exception type here rather than reusing a Core one.
/// </summary>
