...
/// <summary>Supplies the Geocodio API key without prescribing where it is stored.</summary>
public interface IGeocodioApiKeyProvider
{
...
/// <summary>
/// Reads the Geocodio API key from the <c>GEOCODIO_API_KEY</c> process environment variable. Delegates to
...
/// value in the shared <see cref="ApiKey"/> directly -- no dedicated <c>GeocodioApiKey</c> wrapper type is
/// introduced, per <see cref="ApiKey"/>'s own doc comment describing exactly this future-provider case.
/// </summary>
public sealed class EnvironmentGeocodioApiKeyProvider : IGeocodioApiKeyProvider
{
...
public sealed class StaticGeocodioApiKeyProvider : IGeocodioApiKeyProvider
{
    private readonly ApiKey? key;

    public StaticGeocodioApiKeyProvider(ApiKey? key)
    {
        this.key = key;
    }
