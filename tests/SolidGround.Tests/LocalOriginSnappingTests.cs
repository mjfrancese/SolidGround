...
public sealed class LocalOriginSnappingTests
{
    ...
    {
        Coordinate3D candidate = new(719347.6, 4286525.9, 183.99);

        Coordinate3D snapped = LocalOriginSnapping.SnapToWholeSourceUnit(candidate, ProjectedReference(), VerticalReference());

        Assert.Equal(719347d, snapped.X);
        Assert.Equal(4286525d, snapped.Y);
    ...
    {
        // Deliberately near a whole-number boundary (719999.9999999, one part in 1e10 below 720000) to prove
        ...
        Coordinate3D candidate = new(719999.9999999, 4286525.00000001, 183.5);
    ...
    [Theory]
    [InlineData(719347.6, 4286525.9, 183.99)]
    [InlineData(719347d, 4286525d, 183d)]
    ...
    public void SnappedOriginFeedsLocalCoordinateFrameForABitExactRoundTrip()
    {
        ...
        Coordinate3D source = new(719400d, 4286600d, 190d);
