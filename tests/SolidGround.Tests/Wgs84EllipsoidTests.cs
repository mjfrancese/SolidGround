...
public sealed class Wgs84EllipsoidTests
{
    ...
    {
        double expected = ReferenceMetersPerDegreeLatitude(RobandeeLatitude);
        double actual = Wgs84Ellipsoid.MetersPerDegreeLatitude(RobandeeLatitude);

        AssertRelativelyClose(expected, actual, 1e-9);
        AssertRelativelyClose(111009.7668d, actual, 0.0001d);
    }

    [Fact]
    public void MetersPerDegreeLongitudeMatchesAnIndependentReimplementationAndTheDocumentedReferenceValue()
    {
        double expected = ReferenceMetersPerDegreeLongitude(RobandeeLatitude);
        double actual = Wgs84Ellipsoid.MetersPerDegreeLongitude(RobandeeLatitude);

        AssertRelativelyClose(expected, actual, 1e-9);
        AssertRelativelyClose(86990.7935d, actual, 0.0001d);
    }
