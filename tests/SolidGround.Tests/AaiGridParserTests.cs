...
public sealed class AaiGridParserTests
{
    ...
    [Fact]
    public void ParsesTheOfflineRobandeeFixtureWithNorthToSouthRowsAndANoDataHole()
    {
        string path = Path.Combine(AppContext.BaseDirectory, "Fixtures", "robandee-synthetic.asc");
        ...
        Assert.Equal(new Core.Geometry.Coordinate2D(719347.5d, 4286527.5d), grid.GetCellCenter(0, 0));
        Assert.Equal(new Core.Geometry.Coordinate2D(719347.5d, 4286525.5d), grid.GetCellCenter(2, 0));
    }
