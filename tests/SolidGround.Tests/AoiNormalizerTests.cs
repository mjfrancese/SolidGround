...
public sealed class AoiNormalizerTests
{
    ...
    private const double RobandeeLongitude = -90.477652d;
    ...
    {
        Wgs84BoundingBoxAoi bbox = new(-90.48d, 38.69d, -90.47d, 38.71d);
    ...
    public void PadEnvelopeNeverUnderCoversTheTrueGeodesicDistanceForALargeNonStraddlingPad()
    {
        ...
        Wgs84BoundingBoxAoi bbox = new(-90.48d, RobandeeLatitude, -90.47d, RobandeeLatitude + 0.02d);
        LinearDistance margin = LinearDistance.Meters(50000d);
        AoiNormalizationOptions options = new() { EnvelopeMargin = margin };

        NormalizedAoi result = AoiNormalizer.Normalize(bbox, options);

        double trueSouthPadDistance = TrueMeridianDistanceMeters(result.FetchEnvelope.SouthLatitude, RobandeeLatitude);
        double trueNorthPadDistance = TrueMeridianDistanceMeters(RobandeeLatitude + 0.02d, result.FetchEnvelope.NorthLatitude);
        Assert.True(
    ...
    {
        Wgs84RadiusAoi radiusAoi = new(RobandeeLatitude, RobandeeLongitude, LinearDistance.Meters(100d));

        NormalizedAoi result = AoiNormalizer.Normalize(radiusAoi);

        double expectedDLon = 100d / Wgs84Ellipsoid.MetersPerDegreeLongitude(RobandeeLatitude);
        double expectedDLat = 100d / Wgs84Ellipsoid.MetersPerDegreeLatitude(RobandeeLatitude);
        AssertRelativelyClose(RobandeeLongitude - expectedDLon, result.FetchEnvelope.WestLongitude);
        AssertRelativelyClose(RobandeeLongitude + expectedDLon, result.FetchEnvelope.EastLongitude);
        AssertRelativelyClose(RobandeeLatitude - expectedDLat, result.FetchEnvelope.SouthLatitude);
        AssertRelativelyClose(RobandeeLatitude + expectedDLat, result.FetchEnvelope.NorthLatitude);
    ...
    {
        Wgs84RadiusAoi radiusAoi = new(RobandeeLatitude, 179.9999d, LinearDistance.Meters(1000d));
    ...
    {
        Wgs84RadiusAoi radiusAoi = new(89.9995d, RobandeeLongitude, LinearDistance.Meters(1000d));
    ...
    public void GeographicParcelFetchEnvelopeIncludesBufferPlusMargin()
    {
        ParcelGeometryAoi parcelAoi = new(
            ...
            "POLYGON ((-90.48 38.69, -90.47 38.69, -90.47 38.71, -90.48 38.71, -90.48 38.69))",
            GeographicReference(),
        ...
        double expectedDLon = padMeters / Wgs84Ellipsoid.MetersPerDegreeLongitude(38.71d); // larger |lat|
        double expectedDLat = padMeters / Wgs84Ellipsoid.MetersPerDegreeLatitude(0d); // equator: global minimum, always used
        Assert.Equal(-90.48d - expectedDLon, result.FetchEnvelope.WestLongitude, 9);
        Assert.Equal(-90.47d + expectedDLon, result.FetchEnvelope.EastLongitude, 9);
        Assert.Equal(38.69d - expectedDLat, result.FetchEnvelope.SouthLatitude, 9);
        Assert.Equal(38.71d + expectedDLat, result.FetchEnvelope.NorthLatitude, 9);
    ...
    public void ProjectedParcelWithoutATransformThrowsNamingIssueSix()
    {
        ParcelGeometryAoi parcelAoi = new(
            ParcelGeometryFormat.Wkt,
            "POLYGON ((719385 4286527.555, 719415 4286527.555, 719415 4286572.445, 719385 4286572.445, 719385 4286527.555))",
            ProjectedReference());
