...
An opt-in live test (in a separate file, not part of the default offline suite) calls the real endpoint for a small bounding box around the Robandee Lane scenario (`38.700186, -90.477652`, roughly ±0.0007°) and asserts a successful acquisition. It never prints the key or the request URI. It skips (via `Assert.Skip`) unless **both**:
