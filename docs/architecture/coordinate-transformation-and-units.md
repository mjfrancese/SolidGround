...
2. Parses both strings with ProjNET's `CoordinateSystemFactory.CreateFromWkt`. A full, unmodified compound
   WKT1 `COMPD_CS` string (for example the committed `robandee-synthetic.prj` fixture) returns a
...
transform from `Wgs84WellKnownText` and the committed `robandee-synthetic.prj` fixture, reprojects the
committed `robandee-synthetic-parcel.geojson` parcel Forward into that projected reference with
...
absurdly large (each side under 0.01°, for this ~1,346 m² lot plus a 2 m buffer).
...
`(719385, 4286527.554625536)` comes back as `(-174844493.76014572, 156563873.98870513)`). Fed onward with a
...
**no `TOWGS84` clause**, and the committed `robandee-synthetic.prj` fixture's NAD83/UTM-zone-15N definition
...
ReprojectsTheRealRobandeeGeojsonParcelForwardThenInverseRoundTripsToItsOriginalWgs84Vertices` does exactly
...
**Measured basis** (ProjNET 2.1.0, fixture `PROJCS NAD_1983_UTM_Zone_15N` / `robandee-synthetic.prj`,
...
- Round-trip proj→geo→proj (`Forward(Inverse(x))`): max **8.633e-3 m** at the Robandee fixture's own footprint
...
- Geo→proj→geo (`Inverse(Forward(x))`): max **7.776e-8 deg** at the fixture (independently reproduced exactly,
  including against all four real corners of the committed `robandee-synthetic-parcel.geojson` fixture, whose
...
`ProjNetHorizontalCoordinateTransformFactoryTests.ForwardTransformsTheRobandeeCentroidToItsMeasuredUtmCoordinate`),
`ForwardAgreesWithAnIndependentProjEngineReferenceValue` checks `Forward(-90.477652, 38.700186)` against a
...
| --- | --- | --- |
| PROJ (`pyproj`, EPSG:4269→EPSG:26915) | 719348.9011552825 | 4286526.675106941 |
| ProjNET 2.1.0 (this adapter, `Wgs84WellKnownText` → fixture) | 719348.9010351183 | 4286526.679525353 |
