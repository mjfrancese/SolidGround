...
At the Robandee Lane scenario's latitude, `38.700186`°N, these evaluate to `metersPerDegreeLatitude ≈
111009.7668` m and `metersPerDegreeLongitude ≈ 86990.7935` m (independently recomputed for this issue; the
...
actual distance. Confirmed against the actual runtime at a representative Robandee-latitude envelope (`south
= 38.69°`): a 50 km pad under-covered by about 1.9 m, and a 100 km pad by about 7.7 m — negligible at the
...
`tests/SolidGround.Tests/Fixtures/robandee-synthetic-parcel.geojson` and `.wkt` are the same synthetic
~1,346.72 m² (14,496 sq ft) rectangle — the Lot 191 footprint — in two independent reference forms: WGS 84
degrees (built by converting a fixed meter half-width/half-height with `Wgs84Ellipsoid`'s factors at the
Robandee latitude) and NAD83 / UTM zone 15N meters (`EPSG:26915`, the reference `robandee-synthetic.prj`
already describes). The WKT fixture's easting is near `719400`, not the `545000` figure an earlier draft of
this design suggested: Missouri sits roughly `2.5°` east of UTM zone 15's `-93°` central meridian, so at
`≈86990.79` m per degree of longitude at this latitude, `500000` (the zone's false easting) plus that offset
lands near `719000`–`720000` m, matching the already-committed `robandee-synthetic.asc`/`.prj` pair's
`xllcorner 719347`. `545000` would place the fixture roughly `175` km — about `2°` of longitude — west of the
real site, near `-92.5°`, which this design note treats as a plain error rather than a deliberately different
"plausible" example. Both fixtures are synthetic and illustrative, not a real survey, and (per
